...
class Mitigation {
    ...
    public function getAllMitigations($staff_id = null, $is_admin = false) {
        if ($is_admin) {
            $query = "SELECT mitigations.*, staff.staff_name 
                      FROM mitigations 
                      LEFT JOIN staff ON mitigations.staff_id = staff.id";
            return $this->conn->query($query)->fetchAll(PDO::FETCH_ASSOC);
        } else {
            $query = "SELECT * FROM mitigations WHERE staff_id = :staff_id";
            $stmt = $this->conn->prepare($query);
