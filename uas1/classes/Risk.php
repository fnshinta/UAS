<?php
require_once __DIR__ . '../../includes/koneksi_database.php';
...
class Risk {
    ...
    public function getAllRisks($staff_id = null, $is_admin = false) {
        if ($is_admin) {
            $query = "SELECT risks.*, staff.staff_name 
                      FROM risks 
                      LEFT JOIN staff ON risks.staff_id = staff.id";
            return $this->conn->query($query)->fetchAll(PDO::FETCH_ASSOC);
        } else {
            $query = "SELECT * FROM risks WHERE staff_id = :staff_id";
            $stmt = $this->conn->prepare($query);
    ...
    public function addRisk($data) {
        $query = "INSERT INTO risks 
            (objective, process_business, risk_category, risk_code, risk_event, risk_cause, risk_source, qualitative, quantitative, risk_owner, department, staff_id, created_by) 
            VALUES 
            (:objective, :process_business, :risk_category, :risk_code, :risk_event, :risk_cause, :risk_source, :qualitative, :quantitative, :risk_owner, :department, :staff_id, :created_by)";
